...
# -z means empty
# ! is not
# GITHUB_BASE_REF is the branch to which the PR is raised
# so if GITHUB_BASE_REF is not empty, then fetch that origin branch



# Set the HEAD Git commit (the current commit)
GITHUB_SHA=${GITHUB_SHA:=$(git rev-parse HEAD)}
# GITHUB_SHA is the commit SHA that triggered the workflow (commit number i think)
# So i think this line is getting the specific commit thats being pushed to the repo


# Creates a list of .kicad_pcb and .sch files that have changed since the
# GITHUB_BASE_REF
files=()
# creates an empty files variable
...
# for loop format: for variable in array
# git diff only files that have a certain change (not deleted) will be added to files
# keep_going even if there are errors like missing targets
# 
