...
To update the site, a post request is sent to the localhost:4998/<board_name>/<commit> webpage, where
...
flask run
and then navigate to the localhost:4998/ site on your machine's browser
"""
...
def TableOfContents():
    if request.method == 'POST':
        ...
        buildable_list = postRequest['buildable_list'][-1].split(" ")
        commit_number = postRequest['commit_number']
        buildable_list.sort()
        current_board_counter = -1

        while current_board_counter < len(buildable_list):
            current_board_name = re.split('//|/|:', buildable_list[current_board_counter])[-3]
    ...
    if request.method == 'GET':
        ...
        artifacts = {}
        for row in rows:
            if row[-1] not in artifacts.keys():
                artifacts[row[-1]] = [{row[1]: [row[2], row[3], row[4]]}]
            else:
                artifacts[row[-1]].insert(0, {row[1]: [row[2], row[3], row[4]]})
...
def board_name_commit(board_name,commit):
    ...
    rows = cur.fetchall()
    for row in rows:
        if row[-1] == board_name and row[1] == commit:
            schematic = row[1]
            layout = row[2]
            bom = row[3]
